...
import {
  ...
  getSERPQuerySchema,
  getCompanyProfileSystemPrompt,
  generateCompanyProfileResearchPrompt,
  writeCompanyProfileFinalReportPrompt,
...
function useDeepResearch() {
  ...
  function isCompanyProfileQuery(query: string): boolean {
    return query.includes("company profile") || 
           query.includes("Generate a comprehensive company profile") ||
           query.includes("Local 825 Company Profile Framework");
  }

  function extractCompanyProfileData(query: string): {
    companyName: string;
    county: string;
    reportType: string;
  } | null {
    const companyMatch = query.match(/for\s+([^,\s]+(?:\s+[^,\s]+)*?)\s+operating/);
    const countyMatch = query.match(/in\s+([^,\s]+(?:\s+[^,\s]+)*?)\s+County/);
    const reportTypeMatch = query.match(/Generate a\s+([^,\s]+(?:\s+[^,\s]+)*?)\s+report/);
    
    if (companyMatch && countyMatch) {
      return {
        companyName: companyMatch[1],
        county: countyMatch[1],
        reportType: reportTypeMatch ? reportTypeMatch[1] : "full"
      };
    }
    return null;
  }
  ...
  async function askQuestions() {
    ...
    // Check if this is a company profile query
    const isCompanyProfile = isCompanyProfileQuery(question);
    const companyData = extractCompanyProfileData(question);
    
    let systemPrompt = getSystemPrompt();
    let prompt = generateQuestionsPrompt(question);
    
    if (isCompanyProfile && companyData) {
      systemPrompt = getCompanyProfileSystemPrompt();
      prompt = generateCompanyProfileResearchPrompt(
        companyData.companyName,
        companyData.county,
        companyData.reportType
      );
    }
  ...
  async function writeFinalReport() {
    ...
    let systemPrompt = [getSystemPrompt(), outputGuidelinesPrompt].join("\n\n");
    let finalReportPrompt = writeFinalReportPrompt(
      reportPlan,
      learnings,
      enableReferences
    ...
    if (isCompanyProfile && companyData) {
      systemPrompt = [getCompanyProfileSystemPrompt(), outputGuidelinesPrompt].join("\n\n");
      finalReportPrompt = writeCompanyProfileFinalReportPrompt(
        reportPlan,
        learnings,
        enableReferences
          ? sources.map((item) => pick(item, ["title", "url"]))
