...
import ModeSelector, { ResearchMode } from "@/components/Research/ModeSelector";
...
function Topic() {
  ...
  async function handleCompanyProfileSubmit(data: {
    companyName: string;
    county: string;
    model: string;
    reportType: string;
    ...
    if (handleCheck()) {
      ...
      try {
        ...
        // Create a comprehensive research query for the company profile
        const query = `Generate a comprehensive company profile report for ${data.companyName} operating in ${data.county} County. 
        
        Research Requirements:
        - Company overview, history, ownership structure
        - Facility locations with addresses and employee counts
        - Key executives and leadership profiles
        - Financial performance and revenue trends
        - Current projects and government contracts
        - Union relationships and labor history
        - Safety and compliance records (OSHA violations)
        - Legal issues and litigation history
        - Political connections and contributions
        - Market position and competitive analysis
        
        Generate a ${data.reportType} report following the Local 825 Company Profile Framework. Focus on information valuable for union organizing efforts, including any anti-union activities, safety violations, financial vulnerabilities, and strategic pressure points.`;
  ...
  // Reset mode when starting new research
  useEffect(() => {
    if (taskStore.id === "") {
    ...
    <section className="p-4 border rounded-md mt-4 print:hidden">
      <div className="flex justify-between items-center border-b mb-2">
        <h3 className="font-semibold text-lg leading-10">
          {researchMode === 'company-profile' ? 'Company Profile Generator' : t("research.topic.title")}
        </h3>
        <div className="flex gap-1">
          ...
          >
            <SquarePlus />
             ...
             {/* Mode Selector */}
       {taskStore.id === "" && isFeatureEnabled('enableCompanyProfiles') && (
         <ModeSelector
      ...
      {researchMode === 'company-profile' && taskStore.id === "" ? (
        <CompanyProfile onStartResearch={handleCompanyProfileSubmit} />
      ) : (
