...
import {
  ...
  finalReportPrompt,
  companyProfileSystemPrompt,
  companyProfileResearchPrompt,
  companyProfileReportStructures,
...
export function getCompanyProfileSystemPrompt() {
  return companyProfileSystemPrompt.replace("{now}", new Date().toISOString());
}

export function generateCompanyProfileResearchPrompt(
  companyName: string,
  county: string,
  reportType: string
) {
  const reportStructure = companyProfileReportStructures[reportType as keyof typeof companyProfileReportStructures] || companyProfileReportStructures.full;
  
  return companyProfileResearchPrompt
...
export function writeCompanyProfileFinalReportPrompt(
  plan: string,
  learning: string[],
  source: Source[],
  images: ImageSource[],
  ...
  const companyProfileFinalPrompt = `You are conducting research for Local 825 - International Union of Operating Engineers on ${companyName} operating in ${county} County.

${companyProfileSystemPrompt.replace("{now}", new Date().toISOString())}
...
## Report Structure
Generate a ${reportType} report following this exact structure:
...
7. **Focus on information valuable for union organizing efforts**
8. **Identify any anti-union activities or statements**
9. **Note safety violations or worker complaints**
10. **Highlight financial vulnerabilities or competitive pressures**
11. **Identify key decision-makers for labor relations**
12. **Assess the company's market position and competitive landscape**

${enableCitationImage ? `\n**Including meaningful images from the previous research in the report is very helpful.**\n\n${finalReportCitationImagePrompt}` : ""}
${enableReferences ? `\n\n${finalReportReferencesPrompt}` : ""}

Generate a comprehensive, professional report that would be valuable for Local 825's organizing efforts.`;

  return companyProfileFinalPrompt;
}
